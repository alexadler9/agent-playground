...
class ExecutionStageAgent(
    ...
    override val stageSystemPrompt: String = """
        ...
        - не ограничивайся пересказом плана или отчётом о выполнении;
        - execution artifact должен быть самодостаточным: по нему пользователь или другой агент должен понимать, какой результат был подготовлен;
        - если задача требует конкретный результат, answer должен содержать сам результат, а не только сообщение "готово", "сделано", "реализовано";
        - формат результата должен соответствовать задаче: код, текст, рецепт, инструкция, анализ, список, план, письмо или другой требуемый пользователем формат;
